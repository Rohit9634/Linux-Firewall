...
#include <netinet/in.h>  // Add this for inet_ntoa
...
int main() {
    ...
    pcap_loop(pcap_handle, 0, caught_packet, NULL);  // Changed the count to 0 for infinite loop
...
void decode_ip(const u_char *packet) {
    struct ip_hdr *ip_header = (struct ip_hdr *)(packet + ETHER_HDR_LEN);

    printf("=== IP Header ===\n");
    //printf("Version: %d\n", (ip_header->ip_vhl & 0xF0) >> 4);
...
void decode_ip(const u_char *header_start) {
    ...
    printf("\t( Source: %s\t", inet_ntoa(ip_header->ip_src_addr));  // Use inet_ntoa to convert IP to a readable string
    printf("Dest: %s )\n", inet_ntoa(ip_header->ip_dest_addr));  // Use inet_ntoa to convert IP to a readable string
